...
const ForgotPassword = () => {
  ...
  const passwordRecovery = () =>{
    console.log(email);
    axios.get('http://localhost:50450/api/Admin/Forgotpassword?email='+email).then(res=>setPassword(res.data)).catch(res=>console.log(res));
  }
  ...
  const sendEmail = (e)=>{
    ...
    if(password !==''){
    console.log('hi')
    emailjs.send('service_dvd21xv','template_ok8l6la',{
      Subject: "Password Recovery",
      from_name: 'Abdul Wadood Khan',
      to_name: email,
      message: password,
      reply_to: email,
  },'_9xNf_zK6fouybXnY').then(result=>{
