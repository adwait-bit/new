...
namespace ProjectAPI.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        [HttpPost("Userlogin")]
        public async Task<IActionResult> UserLogin(UserModel userModel, string emailid, string password)
        {
            string ar = await user.UserLogin(userModel, emailid, password);
             return Ok(ar);
            //return RedirectToAction().ShowAllPolicy();

        }
        [HttpGet("GetPolicyByType")]
        ...
        [HttpGet("ShowAllPolicy")]
        public async Task<IActionResult> ShowAllPolicy()
        {
            var policies = await user.ShowAllPolicy();
            return Ok(policies);
